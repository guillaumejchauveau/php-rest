...
class Route
{
    ...
    public function match(RequestInterface $request): ?array
    {
        ...
        $requestPath = RequestHelper::getRequestPath($request);
        if ($requestPath[0] === '/') {
        ...
        $requestQuery = RequestHelper::getQueryParams($request);
        foreach ($this->query as $queryArgName) {
            $params[$queryArgName] = $requestQuery[$queryArgName] ?? null;
        }
        return $params;
    }
